...
<section>
    <header>
        ...
        <p class="mt-1 text-xs font-medium text-slate-500">
            {{ __('Aktualisieren Sie die Kontodaten und die E-Mail-Adresse Ihres Benutzerkontos.') }}
        </p>
    </header>
            ...
            @if (auth()->user() instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! auth()->user()->hasVerifiedEmail())
                <div>
                    <p class="text-sm mt-2 text-slate-800">
                        {{ __('Ihre E-Mail-Adresse ist noch nicht verifiziert.') }}

                        <button wire:click.prevent="sendVerification" class="underline text-sm text-slate-600 hover:text-blue-600 rounded-md focus:outline-none">
                            {{ __('Klicken Sie hier, um den Verifizierungslink erneut zu senden.') }}
                        </button>
                    </p>
